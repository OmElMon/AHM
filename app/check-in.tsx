...
export default function CheckIn() {
  const questions = useMemo(dailyQuestions, []); const [step, setStep] = useState(0); const [done, setDone] = useState(false);
  const { control, watch } = useForm<Answers>({ defaultValues: { mood: 84, sleep: 75, energy: 75, smile: '', water: 65, omar: 100 } });
  ...
  const finish = () => {
    const values = watch(); const score = Math.round((Number(values.mood)+Number(values.sleep)+Number(values.energy)+Number(values.water))/4);
    const moodOpt = questions[0].options?.find(o => o.value === Number(values.mood));
    addEntry({ id: Date.now().toString(), date: new Date().toISOString().slice(0,10), score, mood: moodOpt?.label ?? 'Good', moodEmoji: moodOpt?.emoji ?? '😊', energy: Number(values.energy), stress: 100-Number(values.energy), sleep: Number(values.sleep), note: String(values.smile || '') });
    Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success); setDone(true);
  };
  if (done) return <Screen scroll={false}><View style={styles.done}>
    ...
    <View style={styles.result}><Text style={styles.resultLabel}>TODAY'S GLOW</Text><Text style={styles.resultScore}>{Math.round((Number(watch('mood'))+Number(watch('sleep'))+Number(watch('energy'))+Number(watch('water')))/4)}%</Text><Text style={styles.resultNote}>Omar owes you one celebratory coffee. We don't make the rules.</Text></View>
  ...
  return <Screen scroll={false}>
    ...
    <View style={styles.questionWrap}><Text style={[styles.eyebrow, q.omar && { color: '#A07BAB' }]}>{q.eyebrow}</Text><Text style={styles.title}>{q.prompt}</Text>
...
const styles = StyleSheet.create({
  top:{flexDirection:'row',alignItems:'center',justifyContent:'space-between',paddingTop:14},circle:{width:42,height:42,borderRadius:21,backgroundColor:'#fff',alignItems:'center',justifyContent:'center',...shadow},progressText:{fontFamily:'DMSans_700Bold',fontSize:12,color:colors.muted},track:{height:5,backgroundColor:'#F0E1E3',borderRadius:5,marginTop:22},fill:{height:5,backgroundColor:colors.rose,borderRadius:5},
  questionWrap:{flex:1,justifyContent:'center'},eyebrow:{fontFamily:'DMSans_700Bold',fontSize:11,letterSpacing:2,color:colors.gold,textAlign:'center'},title:{fontFamily:'CormorantGaramond_700Bold',fontSize:38,lineHeight:43,color:colors.ink,textAlign:'center',marginTop:9,marginBottom:38,paddingHorizontal:18},answers:{width:'100%'},
