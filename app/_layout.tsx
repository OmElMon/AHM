...
import { useFonts as useSerifFonts, CormorantGaramond_600SemiBold, CormorantGaramond_700Bold } from '@expo-google-fonts/cormorant-garamond';

const client = new QueryClient();

export default function RootLayout() {
  const [dm] = useDMFonts({ DMSans_400Regular, DMSans_500Medium, DMSans_600SemiBold, DMSans_700Bold });
  const [serif] = useSerifFonts({ CormorantGaramond_600SemiBold, CormorantGaramond_700Bold });
  if (!dm || !serif) return null;
  return <SafeAreaProvider><QueryClientProvider client={client}>
    <StatusBar style="dark" />
    <Stack screenOptions={{ headerShown: false, contentStyle: { backgroundColor: '#FFF9F3' } }}>
